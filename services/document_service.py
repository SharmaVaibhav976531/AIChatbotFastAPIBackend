...
class DocumentService:
    ...
    def upload_document(self, user_id: uuid.UUID, file_content: bytes, original_filename: str, extension: str, mime_type: str):
        logger.info(f"[DOC-SERVICE] Processing upload for: {original_filename}")
        ...
        if existing and existing.status == "COMPLETED":
            logger.warning(f"[DOC-SERVICE] Duplicate document detected: {original_filename}")
            raise ValueError("This exact document has already been uploaded and processed.")
        ...
        logger.info(f"[DOC-SERVICE] ✅ Document saved to DB: {original_filename} (ID: {document.id})")
        return document